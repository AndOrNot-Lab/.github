<p align="center">
  <img src="./assets/hero.svg" width="100%" alt="AndOrNot Lab - practical AI, automation and developer tools" />
</p>

<p align="center">
  <img src="./assets/logo.png" width="96" height="96" alt="AndOrNot Lab logo" />
</p>

<p align="center">
  <strong>把想法连接起来，为替代方案留门，删掉没有价值的复杂度。</strong><br />
  <sub>Connect ideas. Keep alternatives open. Remove what does not earn its place.</sub>
</p>

<p align="center">
  <code>AI-native tools</code>&nbsp;&nbsp;·&nbsp;&nbsp;<code>Automation</code>&nbsp;&nbsp;·&nbsp;&nbsp;<code>Developer experience</code>
</p>

## The lab logic

<table>
  <tr>
    <td width="50%" valign="top">
      <img src="./assets/card-and.svg" width="100%" alt="AND - Connect" />
      <h3>连接 / Connect</h3>
      <p>连接信息、模型与工作流，让分散的能力组成真正可用的系统。</p>
      <sub>Join information, models, and workflows into systems people can actually use.</sub>
    </td>
    <td width="50%" valign="top">
      <img src="./assets/card-or.svg" width="100%" alt="OR - Explore" />
      <h3>选择 / Explore</h3>
      <p>保留不同路径，用实验和真实反馈找到更好的答案。</p>
      <sub>Keep multiple paths open, then let experiments and real feedback decide.</sub>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <img src="./assets/card-not.svg" width="100%" alt="NOT - Refine" />
      <h3>删减 / Refine</h3>
      <p>主动删除噪声、脆弱流程和无意义的复杂度。</p>
      <sub>Remove noise, brittle processes, and complexity that has not earned its place.</sub>
    </td>
    <td width="50%" valign="top">
      <a href="https://github.com/orgs/AndOrNot-Lab/repositories">
        <img src="./assets/card-open.svg" width="100%" alt="Open signal - explore public repositories" />
      </a>
      <h3>开放 / Open</h3>
      <p>项目先在内部孵化；当它足够清晰、可靠且可复用时，再走向公开。</p>
      <sub>Projects incubate privately, then open when they are clear, reliable, and reusable.</sub>
    </td>
  </tr>
</table>

## Live signals

<p align="center">
  <a href="https://github.com/AndOrNot-Lab/sourcer/blob/main/LICENSE">
    <img src="https://img.shields.io/github/license/AndOrNot-Lab/sourcer?style=for-the-badge&amp;label=LICENSE&amp;color=0969da" alt="Sourcer license" />
  </a>
  <a href="https://github.com/AndOrNot-Lab/sourcer/network/members">
    <img src="https://img.shields.io/github/forks/AndOrNot-Lab/sourcer?style=for-the-badge&amp;label=FORKS&amp;color=cf4a3c" alt="Sourcer forks" />
  </a>
  <a href="https://github.com/AndOrNot-Lab?tab=followers">
    <img src="https://img.shields.io/github/followers/AndOrNot-Lab?style=for-the-badge&amp;label=FOLLOWERS&amp;color=8250df" alt="AndOrNot Lab followers" />
  </a>
</p>

## Project contributions

<p align="center">
  <a href="https://github.com/orgs/AndOrNot-Lab/repositories">
    <img src="./assets/contributions.svg" width="100%" alt="AndOrNot Lab organization-owned public repository contribution activity over the last 52 weeks" />
  </a>
</p>

<p align="center">
  <sub>Based on organization-owned public repositories. Fork history and bots are excluded; newly opened projects join automatically.</sub>
</p>

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>Toolbox / 工具箱</h3>
      <p>我们按问题选择工具，不为技术栈本身设限。</p>
      <p>
        <a href="https://github.com/AndOrNot-Lab?tab=repositories">
          <img src="https://skillicons.dev/icons?i=python,ts,docker,githubactions&amp;theme=dark" alt="Python, TypeScript, Docker and GitHub Actions" />
        </a>
      </p>
      <sub>Tools follow the problem, not the other way around.</sub>
    </td>
    <td width="50%" valign="top">
      <h3>Build loop / 构建循环</h3>
      <p><code>observe → prototype → test → refine → open</code></p>
      <p>从真实摩擦开始，以可复用的公开成果结束。</p>
      <sub>Start with real friction. End with reusable public work.</sub>
    </td>
  </tr>
</table>

## Lab protocol

> **Useful before novel. Reliable before impressive. Open when ready.**  
> 先解决真实问题，再追求新奇；先做到可靠，再追求惊艳；准备好以后，再开放。

<p align="right">
  <a href="https://github.com/orgs/AndOrNot-Lab/repositories"><strong>Explore public work →</strong></a>
</p>

